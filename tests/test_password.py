...
def test_lookup_pwned_user_error():
    ...
    assert str(user_error.value) == "You must pass either 'passwd' or 'pw_hash'"


@responses.activate
def test_no_user_agent():
    api_url = "https://api.pwnedpasswords.com/range/5BAA6"
    responses.add(responses.GET, api_url, json={"error": "no user agent"}, status=403)
...
def test_hit_rate_limit():
    api_url = "https://api.pwnedpasswords.com/range/5BAA6"
    successful_response = os.path.join(vcr_dir, "pwned_passwords_api_response.txt")
    with open(successful_response, "r") as success:
        success_text = success.read()
    responses.add(responses.GET, api_url, body="error: exceeded rate limit", status=429)
