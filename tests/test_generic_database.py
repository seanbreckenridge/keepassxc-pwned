...
def test_default(capsys, caplog):
    ...
    assert captured_lines[1] == "breached_entry:74B0B92F4476AC6722C2232EB91CB79BC531311C:1054"
    ...
    assert captured_logs[1] == "Found password for 'breached_entry' 1054 times in the dataset!"
    assert captured_logs[2] == "Checking password for unbreached password..."
    assert captured_logs[3] == "Checking password for 3DFE1FD27BDF9870F23C3A5916D9A1495E492518..."  # uses sha1 value since no title
...
def test_verbose(capsys, caplog):
    ...
    assert captured_logs[0] == "Using password from KEEPASSXC_PWNED_PASSWD environment variable"
    assert captured_logs[1].startswith("keepassxc-cli version: ")
    assert captured_logs[2].startswith("Export database command: keepassxc-cli")
    assert captured_logs[3].startswith("Ignoring entry with no password: Credential(title='entry with no password'")
    assert captured_logs[4] == "KeepassXC parsed entry count: 3"
    assert captured_logs[5] == "Checking password for breached_entry..."
    assert captured_logs[6] == "Requesting https://api.pwnedpasswords.com/range/74B0B"
    assert captured_logs[7] == "Found password for 'breached_entry' 1054 times in the dataset!"
    assert captured_logs[8] == "Checking password for unbreached password..."
    assert captured_logs[9] == "Requesting https://api.pwnedpasswords.com/range/92F9D"
    assert captured_logs[10] == "Checking password for 3DFE1FD27BDF9870F23C3A5916D9A1495E492518..."
    assert captured_logs[11] == "Requesting https://api.pwnedpasswords.com/range/3DFE1"
